...
import { validateIdentityToken, upsertSsoUser } from '$lib/server/identity.js';
import { signSession, SESSION_COOKIE } from '$lib/server/auth.js';

export async function GET({ url, cookies }) {
  const token = String(url.searchParams.get('identity_access_token') ?? '').trim();
  const next = url.searchParams.get('next') || '/app/architects';
  ...
  const dest = next.startsWith('/') ? next : '/app/architects';
