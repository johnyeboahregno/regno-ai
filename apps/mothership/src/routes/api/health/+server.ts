...
async function computeServed(): Promise<{ executions: number; servedPhases: number; llmCalls: number }> {
  ...
  return { executions: row.executions, servedPhases: row.servedPhases, llmCalls: row.llmCalls };
}

async function computeUsage(): Promise<unknown> {
  const db = await getDb();
...
export async function GET() {
  // Ping all four stores and run the two best-effort aggregations in parallel,
  // each capped at 2.5s, so a slow/unreachable DB can't stall the response past
  // the deploy validation window (CI curls this endpoint with a short timeout).
  const [redis, mongo, qdrant, neo4j, served, usage] = await Promise.all([
    withTimeout(getRedis().ping().then(() => true).catch(() => false), 2500, false),
    withTimeout(
      (async () => {
        const db = await getDb();
    ...
    withTimeout(neo4jRun('RETURN 1 AS x').then(() => true).catch(() => false), 2500, false),
    withTimeout(computeServed().catch(() => null), 2500, null),
    withTimeout(computeUsage().catch(() => null), 2500, null),
  ]);
