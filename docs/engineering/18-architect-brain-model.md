...
- **Learning + serving (2026-08-30)** — in the current single-brain/SMA model, **Recall & Serve**
  (`docs/architecture/RECALL_SERVE_DECISION_LAYER.md`) still applies: executions write wisdom to
  `cortex_wisdom` (shared knowledge, boosted by SMA focus tags), and the orchestrator serves
  high-confidence matches instead of calling the LLM. Developer isolation keeps one developer's
  learned flavour from being served to another.
